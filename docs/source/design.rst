...
.. spec:: 電源回路設計
   ...
   * **保護:** USB接続時の電源逆流防止および、NiMH 4本直列時の過電圧（充電直後最大約5.8V）からXIAO搭載レギュレータ（XC6210B332MR、入力上限6V）を保護するため、ショットキーダイオードを使用する。
   ...
   * **Diode:** 1N5817 (ショットキーダイオード, 1A, Vf≈0.3V)
       * 目的1: 電圧降下（約0.3V）により、満充電時の5.8Vをレギュレータ入力上限6V以内に収める（5.8V→5.5V）。1N4007（Vf=0.7V）からの変更で電池寿命を改善。
...
.. spec:: 電圧測定回路設計
   ...
   .. code-block:: text
       ...
       (Car Battery +)
             ...
             [R1]  100kΩ
              ...
              |
              +-----> [D1] Zener Diode (3.3V) -> GND (Overvoltage Protection)
              |
              +-----> [C1] Capacitor (0.1uF) -> GND (Noise Filter, RC LPF with R1)
              |
             [R2]  22kΩ
              |
   ...
   * R1: 100kΩ（確定）
   * R2: 22kΩ（確定）
   * D1: BZX55C3V3 ツェナーダイオード 3.3V 500mW DO-35。GPIO絶対定格（3.6V）に対しマージンを確保するため3.3Vを選定。
...
.. spec:: 中継器設計
   ...
   **処理フロー:**
   ...
   1. 起動時にWiFiに接続し、NTP（ntp.nict.jp）で時刻同期する
   2. BLE Coded PHYでセンサーのアドバタイズパケットを常時スキャン
   3. パケット受信時、NTP時刻を基準にタイムスタンプを付与
   4. パケット内の過去ログを展開し、測定間隔から各ログのタイムスタンプを推定
   5. Google Apps Script (GAS) WebhookへHTTP POSTし、スプレッドシート/CSVに追記

   **クラウド連携方式: GAS Webhook**

   * ESP32からGASのWebアプリURLへHTTP POSTするだけで、認証処理が不要（URLが秘密鍵代わり）。
   * サービスアカウントやOAuthはESP32のメモリ・実装コストが大きいため不採用。
   * WiFi SSID/パスワードおよびGAS URLは ``secrets.h`` に定義し、Git管理外とする。

   **タイムスタンプ重複排除:**

   * センサーは毎回パケットに過去ログを含むため、同じ測定データが複数回受信される。
   * リピーターは ``lastUploadedTimestamp`` を保持し、それより新しい推定時刻のエントリのみアップロードする。
   * これにより、BLEパケット欠落時の過去ログ補完と、重複登録の防止を両立する。

   **WiFi切断時の挙動:**

   * WiFi切断中はGASへの送信をスキップする。ローカルバッファリングは行わない。
   * WiFi復帰後、次のBLEパケット受信時に過去ログが含まれるため、データは自動的に補完される。

   **CSVフォーマット:**

   .. code-block:: text

       timestamp,voltage_mv
